  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-80 flex-col bg-[#161616] shadow-xl">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto pl-2 pr-4 py-4 space-y-6">
            ...
            <div className="flex items-center gap-1.5 mb-2 pl-2">
             ...
             <div className="space-y-0.5">
              {aiOptions.map((option) => (
                <label
                  key={option.value}
                  className={`flex items-center gap-1.5 px-2 py-2 rounded cursor-pointer transition-colors text-sm ${
                    aiCommand === option.value
                      ? "text-[#CCCCCC] bg-white/[0.08]"
                      : "text-[#CCCCCC] hover:text-white hover:bg-white/[0.06]"
                  }`}
                >
                  <input
                    type="radio"
        ...
        {/* Footer - Tauri only */}
        {checked && isTauri && (
          <div className="border-t border-[#333333] pl-2 pr-4 py-4 shrink-0">
            <button
              onClick={() => setShowSecurityModal(true)}
              className="flex w-full items-center justify-start gap-1.5 rounded bg-transparent px-2 py-2.5 text-sm text-white hover:text-white hover:bg-white/[0.06] transition-colors"
            >
